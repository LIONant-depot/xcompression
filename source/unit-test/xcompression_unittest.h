...
namespace xcompression::unit_test
{
        ...
        // Test input-driven streaming (fixed_block_compress/fixed_block_decompress, double-buffer simulation)
        {
            ...
            while (true)
            {
                ...
                if (err)
                {
                    ...
                    else if (err.getState() != xcompression::err::state::NOT_DONE)
                    {
            ...
            for (const auto& block : streamCompressedBlocks)
            {
                ...
                while (err && err.getState() == xcompression::err::state::NOT_DONE)
                {
    ...
    void TestFixedBlock(std::span<const std::byte> Source)
    {
        ...
        //
        {
            ...
            std::cout << "Block mode decompressed size: " << verifiedDecompressed.size() << " (matches original)\n";
        }
    }
    //-------------------------------------------------------------------------------------------------------------
    void TestDynamicBlock(std::span<const std::byte> Source)
    {
        ...
        //
        {
            ...
            std::cout << "Dynamic block mode decompressed size: " << verifiedDecompressed.size() << " (matches original)\n";
        }
    }
    ...
    void TestDynamicInputDrivenStreaming(std::span<const std::byte> Source, const std::size_t BlockSize)
    {
        ...
        // Test input-driven streaming (dynamic_block_compress/dynamic_block_decompress, double-buffer simulation)
        {
            ...
            for (const auto& block : streamCompressedBlocks)
            {
                ...
                if (err && err.getState() == xcompression::err::state::NOT_DONE)
                {
                    continue;
                }
                if (err)
    ...
    void RunAllUnitTest()
    {
        ...
        do
        {
            ...
            for (size_t i = 0; i < count; ++i)
            {
                ...
                source.push_back(std::byte{ 'A' }); // Compressible pattern
            }
            count = dis(gen);
            for (size_t i = 0; i < count; ++i)
            {
                if (source.size() >= SourceSize) break;
                source.push_back(std::byte(static_cast<unsigned char>(dis(gen)))); // Random, possibly incompressible
            }
